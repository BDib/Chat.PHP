...
class WordFilter
{
    ...
    public static function isPronounceable(string $name): bool
    {
        ...
        $vc = preg_match_all('/[aeiouy]/i', $name);
        if (mb_strtolower(mb_substr($name, 0, 1)) === 'y') $vc = max(0, $vc - 1);
        if ($vc === 0 || $vc / $len >= 0.8) return false;

        // For structural analysis, treat non-initial y as vowel
        $w = mb_substr($name, 0, 1) . str_replace(['y', 'Y'], 'a', mb_substr($name, 1));
        if (preg_match('/[aeiou]{3}/i', $w)) return false;
        if (preg_match('/(.)\1{2}/u', $name)) return false;
        ...
        preg_match_all('/[bcdfghjklmnpqrstvwxyz]+/i', $w, $clusters);
    ...
    public static function isAllowedWord(string $word): bool
    {
        ...
        if (mb_strlen($word) <= 1) return $word === 'i' || $word === 'a';
        ...
        foreach ($suffixes as $suffix) {
            ...
            if (mb_strlen($word) > $len + 2 && str_ends_with($word, $suffix)) {
                $stem = mb_substr($word, 0, -$len);
                if (isset($allowedWords[$stem])) return true;
                if (isset($allowedWords[$stem . 'e'])) return true;
                if (mb_strlen($stem) >= 2 && mb_substr($stem, -1) === mb_substr($stem, -2, 1)) {
                    if (isset($allowedWords[mb_substr($stem, 0, -1)])) return true;
                }
            }
        }

        // Handle -ied -> -y, -ies -> -y
        if ((str_ends_with($word, 'ied') || str_ends_with($word, 'ies')) && mb_strlen($word) > 4) {
            $stem = mb_substr($word, 0, -3) . 'y';
            if (isset($allowedWords[$stem])) return true;
        }
